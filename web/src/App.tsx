...
import { 
  ...
  Radio, Award, PlayCircle,
  Sparkles, Compass, Database, X, Check
} from 'lucide-react';
...
export function App() {
  ...
  const handleRequestRide = async (e: React.FormEvent) => {
    ...
    const clientId = 'cliente_' + Math.random().toString(36).substring(2, 6);
  ...
  const handleAcceptRide = async (rideId: string) => {
    ...
    try {
      const res = await fetch(`/api/rides/${rideId}/accept`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ driverId: 'motorista_vip_01' })
      });
      ...
      <header style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            >
              <Database size={14} />
              <span>{supabaseConnected ? 'Supabase Conectado' : 'Configurar Supabase'}</span>
            </button>
          </div>
        </div>
      </header>
          ...
          <div className="glass-panel" style={{ maxWidth: '520px', width: '100%', padding: '28px', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ background: 'rgba(99, 102, 241, 0.1)', padding: '12px', borderRadius: '10px', fontSize: '0.75rem', color: '#cbd5e1' }}>
                💡 <strong>Dica:</strong> O script de criação das tabelas e permissões SQL foi criado no arquivo <code>supabase/schema.sql</code>. Basta rodá-lo no SQL Editor do Supabase!
              </div>
            ...
            <div className="glass-panel" style={{ padding: '28px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                  ...
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Gerencie suas disponibilidades e chamados</p>
                </div>
